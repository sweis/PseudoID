...
PublicKey.prototype.generateToken = function(nickname) {
  var t = this.randomInteger();
  var msg = hex2b64(t.toString(16));
  var msgBytes = b64toBA(msg);
  var msgStr = bytesToStr(msgBytes);
  var myStr = Base64.encode(nickname + ':' + msgStr);
  var m = this.hashAndPad(b64toBA(myStr));
  var b = this.blind(m);
  var s = new BigInteger(sign(b.toString(16)), 16);
  var u = this.unblind(s);
  var sig = hex2b64(u.toString(16));
  return new Token(myStr, sig);
}
